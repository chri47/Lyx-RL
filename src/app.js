...
// MEMORIA TEMPORANEA PER LE TRADUZIONI
...
client.once('ready', async () => {
    ...
    const commands = [
        new SlashCommandBuilder()
          .setName('talk')
          .setDescription('Traduci e invia un messaggio')
          .toJSON()
    ];

    try {
        console.log('Registro comando /talk...');
        await rest.put(Routes.applicationCommands(client.user.id), { body: commands });
        console.log('✅ Comando /talk registrato!');
    } catch (error) {
...
client.on('interactionCreate', async interaction => {
    try {
        // COMANDO /talk - STEP 1: SCEGLI LINGUA TUA
        if (interaction.isChatInputCommand() && interaction.commandName === 'talk') {
            ...
            const selectMenu = new StringSelectMenuBuilder()
              .setCustomId('talk_from_lang')
              .setPlaceholder('🌍 Scegli la lingua del tuo messaggio')
              .addOptions([
            ...
            const row = new ActionRowBuilder().addComponents(selectMenu);
            await interaction.reply({ content: '**Step 1/3:** In che lingua scriverai il messaggio?', components: [row], ephemeral: true });
        }

        // STEP 2: SCEGLI LINGUA TARGET
        if (interaction.isStringSelectMenu() && interaction.customId === 'talk_from_lang') {
            ...
            const selectMenu = new StringSelectMenuBuilder()
              .setCustomId('talk_to_lang')
              .setPlaceholder('🌍 Scegli in che lingua tradurre')
              .addOptions([
                    { label: 'Italiano', value: 'it', emoji: '🇮🇹' },
                    { label: 'English', value: 'en', emoji: '🇬🇧' },
                    { label: 'Español', value: 'es', emoji: '🇪🇸' },
            ...
            const row = new ActionRowBuilder().addComponents(selectMenu);
            await interaction.update({ content: `**Step 2/3:** Lingua di partenza: \`${fromLang}\`\nIn che lingua vuoi tradurre?`, components: [row] });
        }

        // STEP 3: APRI MODAL PER SCRIVERE
        if (interaction.isStringSelectMenu() && interaction.customId === 'talk_to_lang') {
            ...
            const modal = new ModalBuilder()
              .setCustomId('talk_modal')
              .setTitle('Scrivi il messaggio da tradurre');

            const messageInput = new TextInputBuilder()
              .setCustomId('message_input')
              .setLabel('Messaggio')
              .setStyle(TextInputStyle.Paragraph)
              .setPlaceholder('Scrivi qui il messaggio...')
              .setRequired(true);
        ...
        if (interaction.isModalSubmit() && interaction.customId === 'talk_modal') {
            ...
            try {
                ...
                await interaction.channel.send(res.text);
                await interaction.editReply({ content: `✅ Messaggio tradotto e inviato!\n\`${cache.from} → ${cache.to}\`` });
                translationCache.delete(interaction.user.id);
            } catch (error) {
                await interaction.editReply({ content: '❌ Errore nella traduzione. Riprova.' });
            }
        }
