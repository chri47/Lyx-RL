...
.addSubcommand(subcommand =>
   ...
   .setDescription('Crea il pannello ticket LYX')
    ...
    async execute(interaction, client) {
       ...
       .addComponents(
           ...
           .setEmoji('🔄'),
            ...
            return interaction.reply({ content: `Pannello LYX creato in ${channel}`, ephemeral: true });
        }
    ...
    async handleButton(interaction, client) {
        ...
        if (categories[interaction.customId]) {
            ...
            const ticketChannel = await interaction.guild.channels.create({
                ...
                topic: `LYX Ticket | ${interaction.user.tag} | Categoria: ${cat.label} | ID: ${ticketCount}`,
            ...
            const embed = new EmbedBuilder()
       .setAuthor({ name: `LYX Support`, iconURL: interaction.guild.iconURL() })
       .setTitle(`${cat.emoji} Ticket #${ticketCount} - ${cat.label}`)
       .setDescription(`**Grazie per aver contattato LYX!**\n\n> Descrivi il tuo problema nel dettaglio.\n> Lo staff ti risponderà il prima possibile.\n\n**Categoria:** \`${cat.label}\`\n**Utente:** ${interaction.user}`)
       .setColor(0x57F287)
       .setThumbnail(interaction.user.displayAvatarURL()) // Avatar utente a destra
       .setFooter({ text: 'LYX Services', iconURL: interaction.guild.iconURL() })
       .setTimestamp();
       ...
       .addComponents(
           ...
           .setEmoji('🔒'),
            ...
            return interaction.editReply({ content: `✅ Ticket LYX creato: ${ticketChannel}` });
        }
